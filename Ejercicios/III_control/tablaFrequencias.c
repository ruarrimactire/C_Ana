...
// para calcular el número de letras del afabeto (ya que son elementos consecutivos de la tabla ASCII) 
...
main() { // Porque no pongo int main()??? -> depende de si quieres que el programa te devuelva un código o no
	int c[N_ELEMENTOS]; //Matriz? Es un vector
	char car; /*Indice*/

	/*Poner los elementos del vector a 0*/
	for(car='a'; car<='z'; car++)	
		c[car-'a'] = 0; //?? -> 
	...
	printf("Introducir texto. Ctrl+z para finalizar\n");
